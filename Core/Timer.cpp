...
{
    return std::chrono::duration_cast<std::chrono::nanoseconds>((isRunning ? Clock::now() : stopTime) - startTime).count();
}

__int64 Timer::GetNanoDeltaTime() const
{
...
{
    return std::chrono::duration_cast<std::chrono::milliseconds>((isRunning ? Clock::now() : stopTime) - startTime).count();
}

__int64 Timer::GetMilliDeltaTime() const
{
